    ...
    def check_observability_keys(self):
        ...
        if self.ENABLE_OBSERVABILITY:
            if not self.LANGFUSE_PUBLIC_KEY:
                raise ValueError("LANGFUSE_PUBLIC_KEY must be set when ENABLE_OBSERVABILITY is True")
            if not self.LANGFUSE_SECRET_KEY:
                raise ValueError("LANGFUSE_SECRET_KEY must be set when ENABLE_OBSERVABILITY is True")
        return self
